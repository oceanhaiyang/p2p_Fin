    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        if (len == 1) {
            ...
            for (i = 0; i < len; i++) {
                ...
                var p = paper.path(opts.init ? ipath : path).attr({ fill: opts.colors && opts.colors[j] || chartinst.colors[j] || "#666", stroke: opts.stroke || opts.colors[j], "stroke-width": (opts.strokewidth == null ? 1 : opts.strokewidth), "stroke-linejoin": "round" });//change line color
		...
		chart.mousemove = function(f){
            ...
            for (var i = 0; i < len; i++) {
                (function (sector, cover, j) {
                    ...
                    cover.mousemove(function (e) { f.call(o,e); });
                })(series[i], covers[i], i);
            }
            return this;
		}
        ...
        var legend = function (labels, otherslabel, mark, dir) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                chart.labels[i].push(txt = paper.text(x + 20, h, labels[j] || values[j]).attr(chartinst.txtattr).attr({ fill: opts.legendcolor || "#000", "text-anchor": "start",'fill-opacity':'0'}));
